...
&& echo "effective_cache_size = 128MB" >> /etc/postgresql/$PGVER/main/postgresql.conf \
...
CMD service postgresql start && service nginx start & gunicorn -w 8 -k sync --worker-connections 12 -t 10 -b 0.0.0.0:5001 wsgi:app